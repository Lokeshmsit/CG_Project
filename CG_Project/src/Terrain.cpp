...
void Terrain::setShader(Shader *shader) { _shader = shader; }

Terrain::Terrain() : mesh(new OBJMeshUV), meshVAO(new MeshVAO) {}
...
void Terrain::Render(Context &ctx) {
  ...
  glm::mat4 view = ctx.camera->getCameraViewMatrix();
...
void Terrain::rotate(float angle, const glm::vec3 &axis) {
  model = glm::rotate(model, glm::radians(angle), axis);
}

void Terrain::translate(const glm::vec3 &position) {}

void Terrain::scale(const glm::vec3 &scale) {}

Shader &Terrain::getShader() { return *_shader; }
