...
GLuint loadCubemap(const std::string &dirname) {
  ...
  for (unsigned i = 0; i < num_sides; ++i) {
    glTexImage2D(targets[i], 0, GL_SRGB8_ALPHA8, width, height, 0, GL_RGBA,
                 GL_UNSIGNED_BYTE, &(data[i][0]));
  }
...
GLuint loadCubemapMipmap(const std::string &dirname) {
  ...
  for (unsigned i = 0; i < num_levels; ++i) {
    for (unsigned j = 0; j < num_sides; ++j) {
      ...
      unsigned error =
          lodepng::decode(data[i][j], width[i], height[i], filename);
      if (error != 0) {
        std::cout << "Error: " << lodepng_error_text(error) << std::endl;
        std::exit(EXIT_FAILURE);
      }
  ...
  for (unsigned i = 0; i < num_levels; ++i) {
    for (unsigned j = 0; j < num_sides; ++j) {
      glTexImage2D(targets[j], i, GL_SRGB8_ALPHA8, width[i], height[i], 0,
                   GL_RGBA, GL_UNSIGNED_BYTE, &(data[i][j][0]));
    }
  }
  glBindTexture(GL_TEXTURE_CUBE_MAP, 0);
